...
@Database(entities = {DemoGroup.class, Event.class, Studio.class, Offer.class, Stream.class, DemoGroupStream.class}, version = 1, exportSchema = false)
public abstract class GlobalRoomDatabase extends RoomDatabase {
    ...
    public static GlobalRoomDatabase getDatabase(final Context context) {
        if (INSTANCE == null) {
            Log.d("GlobalRoomDatabase", "INSTANCE is null");
            synchronized (GlobalRoomDatabase.class) {
                if (INSTANCE == null) {
                    Log.d("GlobalRoomDatabase", "INSTANCE is null after synchronize");
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(),
                            GlobalRoomDatabase.class, "global_database")
